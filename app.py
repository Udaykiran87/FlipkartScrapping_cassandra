...
def index():
    if request.method == 'POST':
        ...
        try:
            ...
            cassandraClient = CassandraManagement()
            ...
            if cassandraClient.isTablePresent(keyspace_name=cred['keyspace'], table_name=searchString.replace("-","_")):
                response = cassandraClient.findAllRecords(keyspace_name=cred['keyspace'], table_name=searchString.replace("-","_"))
    ...
    try:
        ...
        if collection_name is not None:
            scrapper_object = FlipkratScrapper(executable_path=ChromeDriverManager().install(),
                                               chrome_options=chrome_options)
            cassandraClient = CassandraManagement()
            rows = cassandraClient.findAllRecords(keyspace_name=cred['keyspace'], table_name=collection_name.replace("-","_"))
